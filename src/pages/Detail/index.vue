          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  @click="
                    changeChecked(item.spuSaleAttrValueList, item2, index2)
                  "
                  changepirce="0"
...
export default {
  ...
  methods: {
    //点击哪个属性，就把该属性所在的这一行和点击的那个属性作为参数传过来
    //先把点击的那一行的所有属性变白色
    changeChecked(spuSaleAttrValueList, item, index) {
    ...
    getDetailInfo() {
      this.$store.dispatch("getDetailInfo", this.goodsId);
    },
  },
  computed: {
    ...mapGetters(["categoryView", "skuInfo", "spuSaleAttrList", "imageList"]),
    imageList() {
      return this.skuInfo.skuImageList || [{}]; //写了[{}] ，就不需要在zoom组件里写v-if="imageList[0]"
    },
  },
  components: {
    ImageList,
